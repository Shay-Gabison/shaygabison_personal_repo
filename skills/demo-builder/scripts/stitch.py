...
def _which(*names):
    """Resolve the first available exe, tolerating Windows .exe/.cmd suffixes."""
    for n in names:
        for c in ([n, n + ".exe", n + ".cmd"] if sys.platform.startswith("win") else [n]):
            p = shutil.which(c)
            if p:
                return p
    return None
...
def main():
    ...
    for i, pair in enumerate(pairs):
        if "=" in pair:
            ...
            v, a = pair, ""
        section_path = sec_dir / f"s{i}.mp4"
        s = str(section_path)
        mux(v, a or None, s)
        # ffmpeg concat demuxer treats backslashes as escapes; use POSIX paths (Windows-safe).
        entries.append(f"file '{section_path.as_posix()}'")
