...
async function browser([target, out, secs, scrollPx, zoomArg]) {
  ...
  cp('Local State');
  // SECURITY: copy only what auth needs (cookies). Do NOT copy "Login Data" — it holds saved credentials.
  for (const f of ['Cookies', 'Preferences', 'Secure Preferences', 'Web Data'])
  ...
  try {
    ...
    // SECURITY: always delete the copied auth profile, even if capture throws.
